...
- [x] 23-08-PLAN.md — Migration: Modal overlay → --tr-overlay, 9 danger-ring сайтов → --tr-danger-ring, ChartWidget + 4 auth-экрана → --tr-elev-*, финальный check-tokens.mjs (все 4 правила) + pnpm lint/svelte-check/build
...
3. Бейджи-статусы рендерятся в 5 тонах в вариантах мягкая подложка/сплошной/с точкой/счётчик-пилюля.
...
- [x] 24-01-PLAN.md — Фундамент: токен --tr-accent-text + правка CMP-03/Success Criteria #3 (D-06), подавление переходов при смене темы (D-09)
- [x] 24-02-PLAN.md — Button: транскрипция Buttons.dc (5 вариантов x 2 размера x 6 состояний) + секция витрины «Кнопки»
- [x] 24-03-PLAN.md — Fields: Input/Select/Textarea (токен-фикс) + новые Checkbox/Radio (D-04) + секция витрины «Поля ввода»
- [x] 24-04-PLAN.md — Modal: радиус 12px + elev-3 + секция витрины «Модальное окно»

**Wave 2** *(depends on Wave 1 — требует --tr-accent-text из 24-01)*

- [x] 24-05-PLAN.md — Badge: обратносовместимый appearance-проп (D-08, 0 из 21 call-site) + секция витрины «Бейджи»
- [x] 24-06-PLAN.md — Tabs: новый компонент, underline + segmented (D-05) + секция витрины «Вкладки»

**Wave 3** *(depends on все предыдущие — сборка витрины)*

- [x] 24-07-PLAN.md — Витрина: сборка 5 секций, роут /showcase (admin-only, D-01/D-02/D-03), финальный checkpoint
...
- [x] 25-08-PLAN.md — openPanel()/Tab-branch drill-in state reset (WR-01/WR-02) + select-variant search input keyboard/ARIA wiring (WR-06) (CMP-07)
...
- [x] 26-03-PLAN.md — Table framed/footer + Input iconLeft primitive extensions (D-11) (WIN-02)
...
- [x] 26-07-PLAN.md — StatWidget/ChartWidget/PeriodToggle restyle (D-04/D-14) (WIN-01)
...
- [x] 27-03-PLAN.md — Акты: модалки и detail-таблицы, ре-токенизация (D-04) [wave 1]
- [x] 27-05-PLAN.md — Картриджи: крупные модалки OperationModal+ModelFormModal (D-04) [wave 1]
- [x] 27-06-PLAN.md — Картриджи: форма, виджеты (CompatibilityEditor/ContextMenu/LowStockBanner, D-04) + CartridgeFilters→Tabs (D-05) [wave 1]
- [x] 27-08-PLAN.md — Принтеры: модалки+виджеты, DiscoveryResultsTable→Table (D-04) [wave 1]
...
- [x] 27-09-PLAN.md — Финальный build-гейт + both-theme визуальный UAT (checkpoint) [wave 3]
...
- [x] 28-09-PLAN.md — Пользователи: List/ListRow→Table (D-03) + FormModal (D-04) + UsersPage→PageHeader
...
- [x] 29-01-PLAN.md — Общие артефакты: Input `type='password'` (D-01), AuthShell + FormField (D-02)
...
| 29. Вход и интерфейс сотрудника | v1.2 | 1/4 | In Progress|  |
