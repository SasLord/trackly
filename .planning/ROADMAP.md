...
- [x] **Phase 9: AD-аутентификация и заявки на регистрацию пользователей** — AD-вход через браузер, подтягивание ФИО из AD, заявки на регистрацию с подтверждением админом и опциональным автоприёмом (USR-08..12, REQ-06, SET-10); вынесено из Phase 8 при SPIDR-split 2026-06-18 (planned — 5 plans, 5 waves) (completed 2026-06-20)
...
**Goal:** Включить вход доменных пользователей через Active Directory (вынесено из Phase 8 при SPIDR-split 2026-06-18, чтобы тестировать на реальной Windows-машине в домене после релизного пайплайна): AD-логин через браузер, подтягивание ФИО из AD, заявки на регистрацию незарегистрированных AD-пользователей с подтверждением администратором и опциональным автоприёмом. Пароли AD НИКОГДА не сохраняются. **v1 = `ldap3 simple_bind` (решение D-AD-01 в 09-CONTEXT.md); полный auto-SSO (Kerberos/NTLM) отложен в v2 = ADV-01.** Архитектура `trait AdClient` оставляет место под будущий SSO-адаптер.
**Mode:** mvp
...
- [x] 09-02-PLAN.md — AuthService local→AD login fallback (constant-time preserved) + find_user_any_state + ad_* app_settings readers + AppCtx mock/real switch + V028 ad_subtype migration (USR-08, USR-10)

**Wave 3** *(blocked on Wave 2)*

- [x] 09-03-PLAN.md — Registration/restoration write paths: auto-accept vs pending modes + ad_register admin-only filter + approve-with-role + reject branching + restoration (USR-09, USR-11, SET-10, REQ-06)
...
- [x] 09-05-PLAN.md — UI vertical: login redesign (remember/hint/generic errors/reserved SSO) + Pending/Blocked screens + Active Directory settings tab + admin ad_register approve UI + docs/AD-SETUP.md + human-verify (USR-08/09/10/11, SET-10, REQ-06)
...
| 9. AD-аутентификация и заявки на регистрацию пользователей | 5/5 | Complete   | 2026-06-20 |
...
- [x] 10-03-PLAN.md — D-REQ-01 (own-requests override) + BOLA-фикс (get/get_history ownership check) + D-GATE-03 (employee-scoped dashboard branch); Cases 16-20
...
- [x] 11-02-PLAN.md — D-PRN-01: CreateRequest-гейтед эндпоинт принтеров + сгруппированный по Расположению дропдаун
...
- [x] 12-02-PLAN.md — Service: cartridge_repo wiring в RequestService.transition() для D-06/D-07 + RBAC employee-deny тесты

**Wave 3** *(blocked on Wave 2 completion)*

- [x] 12-03-PLAN.md — Frontend: CartridgeSelect + OperationModal selector/prefill + RequestDetail wiring + human-verify checkpoint

**Gap-Closure Wave Round 3** *(2026-06-24 — 4 открытых гэпа GAP-12-09..12, все в Wave 1, файлы не пересекаются → параллельно)*

- [x] 12-16-PLAN.md — GAP-12-09 (B1): Расположение принтера в строке списка (frontend-only, PrinterListRow.svelte; backend device_location уже отдаётся)
...
- [x] 12-20-PLAN.md — D-20/D-21/D-22: опциональный выбор принтера в cartridge-centric установке (новый PrinterSelect.svelte + OperationModal.svelte: приоритизация совместимых принтеров, блок «Предыдущий картридж» с редактируемыми полями)

**Gap-Closure Wave Round 5** *(2026-06-25 — R4-1/R4-3 из 12-HUMAN-UAT.md: лукап принтера в OperationModal резолвил по printers.id вместо device_id, блокируя блок «Предыдущий картридж» в обоих входах установки, Wave 1)*

- [x] 12-21-PLAN.md — GAP-12-13/DEC-A/DEC-B: новая device-id-keyed read-команда printers_get_by_device_id + фикс лукапа/branching хинта/автозаполнение Расположения (printers_sqlite.rs + tauri_cmds/http/specta_export/role_endpoint_matrix + OperationModal.svelte)

### Phase 13: Редизайн совместимости Принтеры↔Картриджи по уникальному наименованию/типу принтера (не per-device junction) + свёрнутые chip-задачи (drum-state дефолт авто-возврата, лимит списка принтеров)
...
- [x] 13-04-PLAN.md — kind-aware дефолт состояния авто-возврата (фотобарабан→5) + uncapped read списка принтеров (SPEC-13-R7, SPEC-13-R8)
- [x] 13-05-PLAN.md — переписать printer_compatib_* тесты на V005 семантику + suggest_compat_printer на devices.name автокомплит (SPEC-13-R2)
- [x] 13-06-PLAN.md — CompatibilityEditor (single printer-name field) + ModelFormModal единый блок + api.ts очистка (SPEC-13-R3)
- [x] 13-07-PLAN.md — PrinterDetail: read-only агрегаты + блок устройства с DeviceFormModal + установленный картридж по коду (SPEC-13-R4, SPEC-13-R5, SPEC-13-R6)
- [x] 13-08-PLAN.md — OperationModal: D-21 reverse-lookup на V005 printer_name client-side матчинг (SPEC-13-R1, SPEC-13-R2)
