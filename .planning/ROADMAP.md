...
**Success Criteria** (what must be TRUE):
  1. Пользователь может создать устройство (через Tauri-десктоп), заполнив только обязательные поля (Тип, Наименование, Расположение, Статус), при этом для опциональных полей (Модель, Состояние, Расположение и пр.) появляется автокомплит из ранее введённых значений.
...
**Success Criteria** (what must be TRUE):
  ...
  5. Поиск по актам находит совпадения по номеру, ФИО Сдал/Принял, наименованию устройства; свитч-бар Акты / Возвраты / Архив показывает корректные счётчики.
**Plans:** TBD
**UI hint:** yes
...
| 2. Устройства и базовый UI | 3/5 | In Progress|  |
