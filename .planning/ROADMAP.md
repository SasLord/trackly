...
- [x] 30-06-PLAN.md — Дашборд: inset-кольцо на переключателе периода (Gap 1) + min-height:0 на .dashboard-grid, устраняющий скролл всего app-shell (Gap 2)

**Wave 5** *(gap closure round 2 — depends on Wave 4's re-run UAT findings 2026-07-25; все 3 плана независимы друг от друга, файлы не пересекаются, выполняются параллельно)*

- [ ] 30-07-PLAN.md — Дашборд, 3-я итерация: горизонтальный запас у inset-кольца переключателя периода (Gap 1) + Layout.svelte grid-template-rows/.content flex-column, root-cause фикс изолированного скролла (Gap 2)
- [ ] 30-08-PLAN.md — Dropdown, 2-я итерация: убрать синее кольцо на всегда-сфокусированном search-input + встроенная клиентская фильтрация select+flat+searchable (закрывает 'zero filtering' для 11 консьюмеров одним изменением) + searchable={false} на коротком списке «Тип» (Gap 3)
