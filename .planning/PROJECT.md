...
**В работе:** v1.3.3 «Печатные формы и приватность данных». **Фаза 34 «Единая шапка документов»
завершена 2026-08-11** (6 планов, DOC-04/05/06, 4/4 must-have). Все три печатные формы рендерят
общий партиал `_header.html` из in-memory реестра шаблонов; добавлено поле
`org_settings.full_name` с экранированием escape-then-`<br>`; механизм доставки обновлений
заменяет файлы, совпадающие с текущим дефолтом или известным legacy-снапшотом (`_legacy_defaults/v21/`),
и намеренно не трогает файлы, правленные вручную, — обе ветки подтверждены на живой установке.
Живой UAT нашёл нерусифицированный период в подзаголовке отчётов (исправлен), code review — 17
находок, включая безусловный `<br />` в шапке при пустом `full_name` (исправлен, покрыт
regression-тестом). PRIV-01: фаза вычистила реальные реквизиты из demo-контекста превью и
добавила CI-гейт против их повторного появления в фикстурах; в истории git они остаются —
согласно уже принятому решению историю не переписывать (см. Key context вехи), остаточный риск
принят осознанно. Системная зачистка HEAD — Фаза 37.
...
## Current Milestone: v1.3.3 «Печатные формы и приватность данных»
...
**Target features:**
- Единая шапка документов (лого + реквизиты организации, Times New Roman 12pt, A4 20/15 мм)
  во всех трёх печатных формах — `act_handover.html`, `act_acceptance.html`, `report.html` —
  и она же дефолт, поставляемый в бинарнике. Название организации берётся из `org.name`,
  хардкод реального названия недопустим.
- Переработка тела акта приёма-передачи: без полосок-подчёркиваний (текст подставляется
  автоматически), полоски только под подписи; горизонтальный блок подписей по строке на
  подписанта с автоподстановкой ФИО; текст акта в каноничной форме (две стороны,
  «составили настоящий акт о нижеследующем», перечень, состояние, срок, подписи).
- Многостраничность акта: одно устройство — всё на первом листе, акт в один лист;
  несколько — на первом листе только имена и отсылка к «Приложению №1», со второго листа
  «Приложение №1» с таблицей полного описания.
- Обезличивание HEAD от уже утёкших реальных данных + durable-гейт против повторной утечки.
- Закрытие Nyquist-покрытия фазы 32.

**Key context:**
- Правки шапки, сделанные пользователем, существуют только в `target/debug/templates/`
  (gitignored, уничтожаются `cargo clean`) — перенос в `crates/trackly-app/templates/`
  обязателен первым действием.
- У файловых шаблонов нет метки «тронут пользователем»; апгрейд сравнивает файл на диске
  со срезом `templates/_legacy_defaults/v20/`. Без нового среза существующие установки
  новую шапку не получат (`crates/trackly-app/src/pdf/html_templates.rs:44-80`).
- `act.giver_name` и `act.receiver_name` уже есть в контексте рендера
  (`act_service.rs:2639-2640`) — автоподстановка ФИО не требует правок бэкенда.
- История git НЕ переписывается (решение пользователя 2026-08-08): чистим только HEAD,
  репозиторий остаётся публичным, остаточный риск принят осознанно.
- Изменения шаблонов проверять рендером настоящего PDF/превью — тесты извлечения текста
  не видят наложение и переполнение.

**Долг, унаследованный из v1.3** (`milestones/v1.3-MILESTONE-AUDIT.md`):
- ~~Предикат `request_type != 'ad_register'` продублирован в трёх SQL-строителях~~ —
  ЗАКРЫТО 2026-08-08 квиком `260808-np4` (2 общие функции, один литерал во всём `src/`).
- Phase 32 — единственная фаза вехи без подтверждённого Nyquist-покрытия
  (`32-VALIDATION.md`: `nyquist_compliant: false`). **Взято в v1.3.3.**
- ~~SSO-01 не покрывает вход по паролю~~ — ПЕРЕКЛАССИФИЦИРОВАНО 2026-08-08 в осознанное
  ограничение, не долг. Форма логин/пароль обслуживает **локальные** учётки, которые выдаёт
  администратор (недоменные машины по LAN + отдельные учётки специалиста/администратора для
  входа с чужих доменных машин). ФИО таких учёток ведёт администратор, каталог к ним
  отношения не имеет. Ветка `try_ad_login` остаётся запасным выходом: если доменный
  пользователь всё же введёт AD-данные в форму, он заведётся с ролью «Сотрудник» и именем,
  которое не обновляется — поведение из Phase 9, вне рабочего сценария. Не чинить, если
  сценарий не изменится.
...
---
*Last updated: 2026-08-18 — **Фаза 37 «Приватность данных» завершена** (веха v1.3.3, 4 плана в 2 волнах, PRIV-01/PRIV-02, верификация 5/5). HEAD очищен от реальных реквизитов организации, ФИО сотрудников и инфраструктурных идентификаторов (18 файлов), бинарные reference-артефакты выведены из индекса. Построен и включён единый durable-гейт `scripts/check-privacy.mjs` (allowlist-режим, поглотивший старый bash-скрипт + SHA-256 n-gram хэши + контроль бинарных расширений, fail-closed): работает в двух точках — локальный `.githooks/pre-commit` (активируется одной командой `./scripts/setup-hooks.sh`, см. CONTRIBUTING.md) и шаг в `ci-fast.yml` сразу после Checkout. Полнота списка токенов доказана против до-очистных блобов истории: 18/18 файлов срабатывают, все 19 боевых токенов достижимы. Открыто: CR-01/CR-02 из code review (`--add` может создать недостижимую запись для значений с внутренней пунктуацией или >3 слов; нечитаемые файлы молча выпадают из скана — fail-open) — на текущий список не влияют, но требуют исправления; PRIV-03 (перезапись истории git) сознательно отложен, поэтому гарантия фазы — «чисто на HEAD + повторный занос блокируется», а не «чисто в репозитории». Следующая фаза — 38 «Nyquist-покрытие Фазы 32».*
