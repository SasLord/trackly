...
**Shipped:** v1.1.1 (2026-07-07) — акт приёма-передачи по образцу Word (мультиустройство) + переход печати актов, отчётов и редактора шаблонов на HTML-печать (браузерный диалог, desktop + LAN); `krilla`/`DocSpec` выведены из активного пути. Ранее: v1.0 + v1.1 (фазы 1–13).
...
## Current Milestone: v1.1.2 Пост-релизные доработки UX и печати
...
**Прогресс:** Phase 18 (Автокомплит и дропдауны) завершена 2026-07-11 — все AUTO-01..05
validated: portal-рендер всех автокомплитов в `body` (не обрезаются в модалках), выбор
устройства в актах с открытием по фокусу, FTS-фильтрацией, группировкой name+model, drill-in
по экземплярам и схлопыванием единственной группы. Далее: Phase 19 (дата акта + редактирование).
...
---
*Last updated: 2026-07-07 — milestone v1.1.2 started: пост-релизные доработки UX (portal-дропдауны, группировка устройств в актах, редактирование актов) и печати (org-контекст device-акта, SVG-логотип, вторая строка адреса) + формат автокодов картриджей/фотобарабанов. v1.1.1 (акт по образцу Word + HTML-печать) завершён 2026-07-07.*
