...
---
*Last updated: 2026-08-12 — **Фаза 35 «Тело акта приёма-передачи» завершена** (веха v1.3.3, 7 планов в 6 волнах, DOC-07/08/09, верификация 5/5, human-UAT пройден на обоих транспортах для обоих актов). Тело акта приведено к каноничной форме Word-образца: интро с двумя сторонами, перечень устройств, состояние, «Сроком до», горизонтальный блок подписей с автоподстановкой ФИО из `act.giver_name`/`act.receiver_name`; полоски-подчёркивания остались только под рукописными подписями. Две волны gap-closure: 35-06 (SC#2 — атрибуция полей своему устройству при N>1) и 35-07 (SC#4/DOC-08 — перенос длинного кириллического ФИО вместо ухода за печатную ширину). Открыто: PRIV-01 (реквизиты в истории git — Фаза 37), бейдж `templates_status` (DOC-12), WR-01 из code review (в `KNOWN_LEGACY_DEFAULTS` зарегистрировано одно из четырёх промежуточных тел фазы — риск для dev/UAT-машин, выпущенные сборки не затронуты). Следующая фаза — 36 «Пагинация акта по количеству устройств».*
