...
### D-05 — Источник реквизитов организации для act-рендера: перейти на `org_settings`
- **Развилка (найдена pattern-mapper'ом, не была видна на этапе discuss):** act-рендер
  сейчас читает `org.*` из **org.json** (`OrganizationService::read` → `OrgData`, поля
  только name/inn/kpp/address), тогда как Settings UI пишет реквизиты в **таблицу
  `org_settings`** (`OrgDbService`). Новые phone/fax/email/okpo/ogrn без моста не дойдут
  до контекста акта.
- **Решение пользователя — Option A:** переключить act-рендер на
  `OrgDbService::get_for_pdf()` (`OrgSettingsDto`) как единый источник реквизитов.
  Прецедент: `crates/trackly-app/src/services/report_service.rs` L522 уже строит
  `HeaderBlock` из `OrgSettingsDto`. Это унифицирует источник org-данных и делает так,
  что шапка акта отражает то, что редактируется в Настройках.
- **Backward-compat (success criterion #4):** если org.json и org_settings разошлись по
  name/inn/kpp/address, шапка ранее созданных актов начнёт печатать значения из таблицы
  `org_settings`. Планировщику: считать это ожидаемым (унификация), но обеспечить, что
  отсутствующие/пустые реквизиты деградируют в «—»/пусто, а не в ошибку рендера.
- НЕ выбирать Option B (дублирование полей в OrgData/org.json) — два источника org-данных
  требуют синхронизации и создают риск рассогласования.
