...
Fixes applied to `scripts/check-privacy.mjs` after this review; every claim below was
re-verified by running the gate, not by reading the code.

| Finding | Status | What changed |
|---|---|---|
| CR-01 | fixed | `--add` теперь считает хэш от канонической формы сканера (`canonicalizeAddValue`, общая с `extractNgrams` константа `NGRAM_MAX_WORDS`); значение длиннее окна n-грамм или без буквенно-цифровых слов отклоняется (exit 1, файл не меняется). 19 существующих записей не трогались — они уже достижимы. |
| CR-02 | fixed | Сборщики целей возвращают `{targets, unreadable}`; нечитаемая цель печатается как `путь — не прочитан (код)` и даёт exit 1 во всех трёх режимах. Проверено на ENOENT, EACCES и staged-блобе >64 МБ (ENOBUFS). |
| WR-01 | fixed | `REQUISITE_FILE_RE` расширен на структурированные конфиги и исходники (`json/toml/ya?ml/svelte/ts/tsx/jsx/css/scss/minijinja/sql/txt/csv`); `.md` и `.mjs/.js` исключены сознательно (шаблонные плейсхолдеры и комментарии самого гейта имеют ту же форму). |
| WR-02 | fixed | Флаг `i` у обоих паттернов режима 1. |
| WR-03 | fixed | Вторая ветка паттерна — значение без кавычек (числовой литерал), с отбрасыванием `_` перед сверкой с `ALLOWED`. |
| WR-04 | mitigated | Каталог по-прежнему исключён из авто-сканирования (иначе хук и CI были бы красными навсегда), но self-test теперь фиксирует его состав явным списком: добавление файла обязано пройти через правку теста, т.е. через code review. |
| WR-05 | fixed | `promptHidden` обрабатывает чанк посимвольно и не пропускает управляющие символы в значение. |
| IN-01 | fixed | Комментарий фикстуры описывает актуальное поведение (`REQUISITE_FILE_RE` матчит `.rs.txt`; позиционный вызов нужен из-за `AUTO_SCAN_EXCLUDED_PREFIXES`). |
| IN-02 | fixed | `--add` отказывает при попытке записать тот же хэш под другим классом и не меняет файл при повторе с тем же классом. |
| IN-03 | not addressed | Пин версии Node для CI-шага остаётся открытым (info-level, поведение не меняет). |

Расширение режима 1 (WR-01/02/03) проверено на отсутствие ложных срабатываний: по всему
репозиторию новые формы не дают ни одного дополнительного совпадения, кроме намеренно
нарушающей фикстуры. `check-privacy.selftest.mjs` дополнен регрессиями CR-01, CR-02,
WR-01/02/03 и WR-04 (25 проверок, exit 0); полный прогон по HEAD и прогон в `--staged`
режиме — exit 0.
