...
> Обе вопроса закрыты на этапе планирования (2026-08-11). Аннотации ниже — указатели на
> план/задачу, которые их реализуют.

1. **Нужен ли структурный тест-гейт «в теле handover нет `border-bottom`» (по образцу Фазы 33
   D-13)?**
   - **RESOLVED: да, гейт заводится.** План `35-04`, Task 3 — новый файл
     `crates/trackly-app/tests/html_field_row_underline_gate.rs`.
     ⚠️ Реализация — **не** по диапазону markup: блок `<style>` (строки 41-106 текущего HEAD)
     содержит оба объявления `border-bottom` и лежит **до** маркера `{% include "_header.html" %}`
     (строка 110), поэтому «диапазон между include и `.signatures`» даёт ноль вхождений по
     построению. Гейт проверяет CSS по точным селекторам: тело правила `.field-row` без
     `border-bottom`, ровно 2 вхождения во всём `<style>`, оба — в `.value-blank` и
...
2. **Требует ли живой предпросмотр в UI Settings → Шаблоны (`TemplateEditor`) отдельного шага
   ручной UAT-проверки?**
   - **RESOLVED: да, включён в UAT.** План `35-05`, Task 2 (`checkpoint:human-verify`,
     `gate="blocking"`) — в `how-to-verify` есть шаг Settings → Шаблоны → `act_handover` /
     `act_acceptance` → «Предпросмотр» без сохранения. Обязательная правка демо-контекста
     (`demo_context_for_kind`, Pitfall 1) — план `35-01`.
