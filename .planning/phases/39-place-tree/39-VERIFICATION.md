...
| `crates/trackly-app/src/tauri_cmds/places.rs` + `http/places.rs` | 12 commands mirrored on both transports | ✓ VERIFIED | 12 HTTP routes mounted (`http/places.rs:307-318`); 12 commands registered via `collect_commands!` in `specta_export.rs`, consumed by `main.rs`'s `.invoke_handler(builder.invoke_handler())`. Transport parity is complete — every command, `places_subtree_stats` included, has a matching `POST /api/v1/places_*` route. **Correction (milestone audit 2026-08-27):** this row previously read "11 HTTP routes" and claimed `places_subtree_stats` had no HTTP route "by design"; that was wrong — the route exists at `http/places.rs:316`, so the original claim understated the delivered integration. |
