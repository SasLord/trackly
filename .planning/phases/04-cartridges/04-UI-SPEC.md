...
| Label | `--font-size-label` | 13px | 400 | 1.4 (`--line-height-label`) |
| Heading | `--font-size-heading` | 20px | 600 (`--font-weight-semibold`) | 1.3 (`--line-height-heading`) |
| Display | `--font-size-display` | 28px | 600 | 1.2 (`--line-height-display`) |

Дополнительно: `--font-weight-medium: 500` допускается для активных вкладок switch-bar и label форм (паттерн Phase 2/3).
...
**Accent reserved for:** активная вкладка switch-bar (статус + тип + модель), активный пункт сайдбара «Картриджи», primary-кнопки CTA («Добавить картридж», «Добавить модель», «Сохранить»), фокус-ring на всех интерактивных элементах.
...
| Контекст | Кнопка |
|---------|--------|
| Создать экземпляр | + Добавить картридж |
| Создать модель | + Добавить модель |
| Сохранить форму экземпляра (новый) | Добавить |
| Сохранить форму экземпляра (редактирование) | Сохранить |
| Сохранить форму модели (новая) | Добавить |
| Сохранить форму модели (редактирование) | Сохранить |
| Подтвердить операцию в OperationModal | Выполнить |
...
- Kebab кнопка (⋮): `width: 28px, height: 28px`, `border-radius: --radius-sm`.
...
- Кнопка «Выполнить» — `variant="primary"`, `loading` во время запроса, блокируется при валидационных ошибках.
...
- Header: код (tabular-nums), название модели (brand+model), статус-badge, состояние заряда.
...
- Kebab ⋮ — контекстное меню только с «Редактировать» / «Удалить».
