...
### UAT Round 4 — 2026-06-13 (3 фикса по фотобарабанам, commit `c5c3dcf`)

- **R4-1** — В детали фотобарабана пряталась кнопка «На заправку» (и «Установить» для
  отработанного) — раньше гейтилось только контекстное меню, не actions-строка детали.
- **R4-2** — Индикатор состояния барабанов был всегда серым: состояния 4/5/6 не
  попадали в маппинг цвета. Теперь Новый→зелёный, Изношенный→янтарный,
  Отработанный→красный (списанные по-прежнему серые).
- **R4-3** — Возврат барабана на склад: поле «Состояние» по умолчанию → «Отработанный».

Status: `svelte-check` (0 errors) + `lint` зелёные. Awaiting UAT Round 5.
