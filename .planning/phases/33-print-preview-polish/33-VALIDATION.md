...
| ~~**LAN-печать: поля и шрифты применились к `#act-print-root`**~~ **ЗАКРЫТО** | PRV-03 | ~~RESEARCH.md Open Question 2 — форма аргумента `Polisher.add()` не подтверждена~~ **Разрешено живым UAT + быстрофиксом `260805-edd`:** правильная форма — объект (`[{ 'act-preview.css': cssText }]`), не строка. Теперь зафиксировано типами в `ui/src/pagedjs.d.ts`, т.е. откат ловится `svelte-check` | — (проверять больше не нужно; регресс ловится автоматически) |
| **Многостраничная пагинация превью (N≥2 страниц)** | PRV-01, PRV-02 | **Этот проект ни разу не наблюдал многостраничное превью работающим end-to-end** (`.planning/debug/resolved/print-preview-always-degrades.md`, ~L1128 — все UAT, включая round 4, проходили только на однолистовых актах). Реальная пагинация, per-sheet рендер тени и отсутствие reflow **полностью не проверены**. Фронтенд-харнесса рендера нет, а синтетический Playwright/Chromium харнесс в этом проекте не принимается как верификация (приложение работает в WKWebView/WebView2) | Открыть предпросмотр акта на **N≥2 устройств** (и длинный отчёт) в desktop-режиме и в LAN-браузере, в обеих темах. Проверить: (1) в превью действительно больше одной страницы; (2) у каждого листа своя тень, лист белый, поля видны; (3) при скролле и смене темы нет reflow и перепагинации; (4) число страниц в футере совпадает с числом листов; (5) печать/Save-as-PDF даёт то же число страниц и те же точки разрыва, что и превью |
