...
## Транспорт-матрица не покрывает Tauri-половину двух поздних эндпоинтов (WARNING-01)

**Найдено:** 2026-09-05, аудитом безопасности фазы (`/gsd-secure-phase 40`); зафиксировано как
WARNING-01 в `40-SECURITY.md`.

**Симптом:** `crates/trackly-app/tests/role_endpoint_matrix.rs` — гейт транспорт-паритета,
инвариант которого объявлен угрозой `40-14/T-40-28`: каждый новый эндпоинт получает и HTTP-, и
Tauri-кейс. Два эндпоинта, добавленных в раундах gap-closure ПОСЛЕ плана 40-14 —
`cartridges_operation_default_place` (план 40-30) и `cartridges_to_refill_last_send` (план 40-33) —
имеют только HTTP-кейсы (Case 60-63).

**Сегодня обхода нет:** гейт `authorize(caller, &Action::ReadData)` живёт в общих
`build_cartridges_operation_default_place` / `build_cartridges_to_refill_last_send`
(`trackly-app/src/tauri_cmds/cartridges.rs:226` и `:240`), а Tauri-обёртки (`:453-467`, `:471-476`)
делают ровно `resolve_tauri_identity` + делегирование. Проверено чтением кода, не предположением.

**Риск регрессионный, не эксплуатационный:** будущая правка, снимающая гейт на Tauri-пути, не будет
поймана матрицей — то есть ослаблен именно тот гейт, который фаза 40 заводила против IN-02.

**При починке:** добавить Case 64/65 по образцу существующих Tauri-half кейсов (Case 53/55/59
вызывают `build_*` напрямую) — Manager-allow / Employee-deny для обеих функций.
