...
4. Расхождения с уже утверждёнными компонентами Фаз 24–25 не «чинятся молча» — они перечислены в
   §8 «Принятые отклонения». Компоненты Фазы 24 — тоже утверждённый контракт, и переписывать их
...
| подпись значения | `font-size:9px; font-weight:600; color:var(--tr-text-secondary)`, `bottom:h%`, `transform:translate(-50%,-3px)` | Д:250 | **CHG** |
...
| 9px | 9px | 600 | — | подпись значения над столбцом графика — **литерал вне шкалы**, только там |
...
только `framed`, полоса итога остаётся снаружи рамки, и это фиксируется в §8 как принятое
...
| Семантика | `--tr-danger-text` | `#b02f2f` | `#ff8080` | текст ошибки загрузки виджета |
...
Всё — по-русски (ограничение v1). Изменяется ровно три строки; остальное сохраняется дословно.

| Элемент | Копирайт | Статус |
|---|---|---|
| Логотип сайдбара | «Trackly» | NEW (D-08) |
| Подпись переключателя тем | «Оформление» | **CHG** — было «Тема» (D-08) |
| Кнопки сегмент-контрола | «Светлая» · «Системная» · «Тёмная» | порядок CHG, тексты те же |
| Кнопка выхода | «Выйти» / «Выход…» | KEEP (D-08) |
| Primary CTA (Устройства) | «+ Создать устройство» | KEEP |
| Secondary (Устройства) | «Импорт CSV» · «Экспорт CSV» | KEEP |
| Primary CTA (Дашборд) | **отсутствует** | D-03 — «+ Создать акт» не заводится |
| Титулы окон | «Дашборд» · «Устройства» | KEEP |
| Пустое состояние (поиск) | «По вашему запросу ничего не найдено» / «Попробуйте изменить поисковый запрос или сбросить фильтр статуса.» | KEEP (Фаза 25) |
| Пустое состояние (нет данных) | «Устройств пока нет» / «Создайте первое устройство или импортируйте список из CSV.» | KEEP (Фаза 25) |
| Итог таблицы | «Показано N из M» / «Групп: N» | KEEP |
| Ошибка виджета | «Не удалось загрузить» | **CHG** — было «Ошибка загрузки»; правка только текста, поведение по D-14 не меняется |
| Предупреждение в карточке | «Низкий остаток:» + список моделей | KEEP (D-04) |
| Табы фильтра | «Все» · «На складе» · «В работе» · «На ремонте» · «Списано» | KEEP |
| Чекбокс группировки | «Группировать похожие» | KEEP |
| Периоды графика | «3 мес.» · «6 мес.» · «12 мес.» | KEEP |
| Бургер-меню | `aria-label` «Открыть меню» / «Закрыть меню» | NEW |
| Пропустить к контенту | «Перейти к основному содержимому» | KEEP |
...
3. **`ROADMAP.md` §Phase 26 — секция `Plans:` содержит копипасту планов Фазы 25** (`25-01`…`25-07`,
   все `[x]`, и `**Plans**: 7 plans`). Очистить до планирования, иначе подсчёт прогресса и
   верификация будут врать.
...
6. **Регресс-проверка двух потребителей `Table`:** список устройств и витрина CMP-06.
7. **Человеческий чекпоинт визуального сравнения с `.dc` — только живой подписью.**
