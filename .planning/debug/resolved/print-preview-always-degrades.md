...
verification: |
  ...
  - МНОГОСТРАНИЧНЫЙ случай — ЭТОТ ПРОЕКТ НИ РАЗУ НЕ НАБЛЮДАЛ многостраничное превью работающим
    ...
    ЗАКРЫТО 2026-08-05 (позже в тот же день, после всех быстрофиксов 260805-*): пользователь
    напечатал акт на 2 страницы — результат корректный. Это первое подтверждённое наблюдение
    работающего многостраничного рендера в проекте. Абзац выше сохранён как есть — он был
    точен на момент написания; не переоткрывать вопрос по нему.
    Запись верификации: .planning/phases/33-print-preview-polish/33-VALIDATION.md,
    раздел «Manual UAT Sign-Off 2026-08-05».
