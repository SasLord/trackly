...
status: executing
last_updated: "2026-09-18T23:07:23.519Z"
...
- [Phase 40.2-05]: NumberTemplateService wired into AppCtx (Plan 04 built it but never connected it) — number_templates field added, 7 hand-built AppCtx literals updated
- [Phase 40.2-05]: action_for_context single pure function maps TemplateContext -> Action, both transports import it; Group A CRUD endpoints accept no context param at all (T-40.2-09 structural mitigation)
- [Phase 40.2-05]: WsEvent::NumberSpaceChanged carries ready contexts: Vec<String> (not a space string) to avoid a client-side Rust<->JS mirrored mapping; real dispatch deferred to Plans 06/07/08
...
| 2026-09-05 | 260904-x7s-act-delete-frees-number | Удаление акта теперь освобождает его номер. Было: `delete_soft` помечает `acts.deleted_at_utc`, но проверка уникальности в `create`/`update` намеренно смотрела **включая** soft-deleted (`SELECT EXISTS(... WHERE number=?1)`, решение D-Soft-vs-Hard-Acts-01), а автономер брался из монотонного `counters.act_number` — после удаления акта №20 ни ручной ввод 20, ни кнопка «Следующий» его не возвращали. Стало (решение пользователя, отменяет только пункт про уникальность из D-Soft-vs-Hard-Acts-01; сам soft-delete и undo через audit_log не тронуты): новая `next_act_number_from_max(conn)` в `acts_sqlite.rs` — `COALESCE(MAX(number),0)+1` среди живых актов (`deleted_at_utc IS NULL`), fallback 1 на пустой таблице; `Transaction` дерефится в `Connection`, поэтому одна функция обслуживает и read-путь (`peek_next_number` → Tauri-команда и HTTP-хендлер через общий сервисный слой), и write-путь (`create` вызывает её с `&tx` в той же транзакции, что и INSERT — гонок нет при single-writer + BEGIN IMMEDIATE). Обе проверки уникальности (`create` и `update`-переименование) получили `AND deleted_at_utc IS NULL` — теперь совпадают с уже существующим частичным индексом `idx_acts_number_sub_unique` (V004). `next_auto_would_be` в аудит-payload `custom:act_number_override` считается ДО INSERT (иначе MAX учёл бы только что вставленную строку). `counters.act_number` больше не читается и не инкрементируется на пути актов; строка и функции-счётчики оставлены — их использует `cartridge_seq`. Возвраты не затронуты: они наследуют `number` родителя, а их `sub_number` уже считался через `next_sub_number_for_parent` с фильтром по живым. Схема БД не менялась, миграции не потребовалось. +2 юнит-теста (`next_act_number_from_max_empty_table_is_one`, `..._ignores_soft_deleted`) и +3 интеграционных (`create` с override на освобождённый номер, `update`-переименование на освобождённый номер, `peek_next_number` возвращает 20 после удаления акта №20). Существующие `override_number_already_exists_returns_conflict` и `number_change_rejects_duplicate` остались зелёными без правок (оба про живые акты). Гейты: acts_crud/acts_update/acts_numbering/acts_returns/acts_display_rule/acts_search/places_contents/place_movements_timeline/report_movements/export_bindings + trackly-infra --lib (149) + seed_data/per_record_invariants — все зелёные; `clippy --workspace --all-targets` чисто; `cargo fmt --check` по затронутым файлам чисто; `check-privacy` — 0 нарушений. | complete ✓ |
| 2026-09-18 | 260918-mtv-place-delete-hard-soft-deleted-fk | Удаление места, на которое ссылаются только soft-deleted записи (находка 40.1-SECURITY, low). Было: предполётная проверка `delete_hard` считает только живые строки, а FK V038 (`ON DELETE RESTRICT`) держат и удалённые — `DELETE` падал, клиенту уходило сырое `FOREIGN KEY constraint failed`. Стало: `places_sqlite::delete_hard` маппит FK-ошибку DELETE (`map_place_delete_error`: ConstraintViolation + текст SQLite; extended code у bundled SQLite = 1811, не 787) в `Conflict` «Место нельзя удалить: на него ссылаются удалённые записи … Архивируйте место.». `SubtreeStats`/UI-счётчики не тронуты. +тест `delete_blocked_by_soft_deleted_device_has_russian_reason` (красный без фикса). Гейты: place*-тесты, infra --lib 149, clippy, fmt — зелёные. | complete ✓ |

## Session Continuity

Last session: 2026-09-18T23:07:23.508Z
Stopped at: Completed 40.2-05-PLAN.md
Resume file: None
