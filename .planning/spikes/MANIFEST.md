...
|---|------|------|-----------|---------|------|
| 001 | sspi-windows-compile | standard | Given Trackly's build, when `sspi` (server-side Kerberos/Negotiate accept) is added behind the AD adapter layer, then it compiles+links for Windows MSVC in CI and `cargo check` on macOS | PENDING | rust, kerberos, sspi, ci, portable |
