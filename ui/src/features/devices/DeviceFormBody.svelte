...
<script lang="ts">
  ...
  //
  // CR-01 (ревью 40.4): условие по инвентарному номеру ОБЯЗАНО совпадать с
  // `quantityDisabled` выше — иначе NEW-3 отменяется сам собой. До 40.4
  // `quantityDisabled` был `true` для ЛЮБОГО непустого номера, поэтому
  // терять пользователю было нечего; после NEW-3 поле «Количество» остаётся
  // активным при автоподставленном номере, и любая ПРОГРАММНАЯ запись в
  // `inventoryNo` (автоподстановка при монтировании, прилетевшая после того
  // как пользователь уже ввёл «Количество»; повторный peek при смене
  // «Устройство»↔«Принтер» — NumberTemplateField's context-$effect →
  // applyContextDefault) молча сбрасывала «Количество» в 1, и вместо N
  // устройств создавалось одно, без единого сообщения.
  //
  // `serialNo` автоподстановки не имеет — его условие остаётся как было.
  // Отслеживаются `inventoryNo`/`inventoryNoEditedByHand`/`serialNo`;
  // `quantity` только ЗАПИСЫВАЕТСЯ, поэтому эффект не будит сам себя.
  $effect(() => {
