<script lang="ts">
  ...
  // ---------------------------------------------------------------------------
  // Выход из попапа — ОДИН путь на все способы закрытия (CR-02, ревью 40.4)
  //
  // К моменту, когда пользователь видит шаг 4, `doImport()` уже закоммитил N
  // строк в БД, а `Modal` зовёт `onClose` на Escape, клике по подложке и
  // кнопке «×» — все три живы и на шаге 4. Поэтому «импорт завершён» не может
  // жить в обработчике одной кнопки «Готово»: раньше любой из этих трёх
  // способов закрытия оставлял список устройств, счётчики статусов и счётчики
  // содержимого дерева «Места» (инвалидация N-3) устаревшими, без единого
  // сообщения. Теперь и «Готово», и любое закрытие идут через ЭТУ функцию —
  // одна ветка, а не две копии логики обновления.
  //
  // `report !== null` — единственный признак состоявшегося коммита: он
  // выставляется только на success-ветке `doImport()` и снимается
  // `resetState()` ниже. `onImported` в DevicesPage сам закрывает попап
  // (`csvModalOpen = false`), поэтому отдельный `onClose()` в этой ветке не
  // нужен.
  //
  // Инвариант держит гейт INV-8 в
  // `ui/scripts/check-place-tree-invalidation.mjs`, а не только этот
  // комментарий.
  // ---------------------------------------------------------------------------
...
<Modal {open} title={stepTitle(step)} size="wide" onClose={handleClose}>
  ...
  {#snippet footer()}
    <div class="modal-footer-actions">
      ...
      {:else if step === 4}
        <!-- CR-02: «Готово» — тот же `handleClose`, что Escape/подложка/«×»
             (см. его доккомментарий): один путь выхода, одна инвалидация. -->
...
<style lang="scss">
  ...
  .preview-table {
    ...
    th {
      ...
      padding: var(--tr-space-xs, 8px) var(--tr-space-md, 12px);
      text-align: left;
      font-weight: var(--tr-font-weight-semibold, 600);
