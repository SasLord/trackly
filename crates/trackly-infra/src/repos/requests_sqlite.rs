...
///   - `place_full_paths pfp` for the printer device's resolved place path
///     via `devices.place_id` (D-05, Phase 12; migrated onto the place tree
///     in Phase 39).
///   - `request_categories rc` for category display name (D-CAT-01, free_form requests).
///
/// `category_name` (idx 18) and `printer_place` (idx 19) are appended
...
const SELECT_REQUESTS: &str = "
    SELECT r.id, r.request_type, r.status,
           ...
           pfp.full_path AS printer_place
      FROM requests r
