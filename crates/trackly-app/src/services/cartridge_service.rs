...
impl CartridgeService {
    ...
    /// 2. Если шаг 1 вернул `None` — `cart_repo.latest_to_refill_source_place(&conn)`
    ///    (тот же conn, тот же `spawn_blocking`, один reader-acquire на обе
    ///    попытки, не два) — место-ИСТОЧНИК САМОЙ СВЕЖЕЙ отправки на
    ///    заправку ЛЮБОГО картриджа, У КОТОРОЙ ЭТОТ ИСТОЧНИК ВООБЩЕ ИЗВЕСТЕН
    ///    (UAT5-02 debug-follow-up,
    ///    `.planning/debug/from-refill-place-looks-filled.md`). До этого
    ///    фикса здесь стоял `cart_repo.latest_to_refill_send(&conn)?.and_then(...)`
    ///    — он берёт РОВНО ОДНУ самую свежую запись `custom:to_refill`
    ///    ЦЕЛИКОМ (это собственный контракт `latest_to_refill_send`, не
    ///    тронутый), и если у ЭТОЙ КОНКРЕТНОЙ записи источник пуст (штатно:
    ///    первое присвоение места по D-06 не пишет движение; данные старше
    ///    этой функциональности), шаг 2 молча возвращал `None`, ДАЖЕ ЕСЛИ у
    ///    более ранней отправки источник был. `latest_to_refill_source_place`
    ///    — осознанно ОТДЕЛЬНЫЙ резолвер (см. его doc-комментарий в
    ///    `cartridges_sqlite.rs`), а не правка `latest_to_refill_send`:
    ///    последняя обязана и дальше отдавать одну самую свежую запись
    ///    целиком для диалога «Отправка на заправку» (все три поля из ОДНОЙ
    ///    отправки, Plan 40-33/UAT4-02) — это другой вопрос, отвечающий на
    ///    другой UI-диалог, и смешивать их снова означало бы повторить
    ///    ошибку UAT3-01a зеркально. Если шаг 1 вернул `Some`, шаг 2 не
    ///    вызывается вовсе — короткое замыкание, собственная история
    ///    побеждает даже если глобальная запись свежее по времени в
    ///    абсолютном выражении.
    /// 3. Иначе — `None`, поле остаётся пустым (симметричная постоянная
    ///    подсказка под полем в `OperationModal.svelte` объясняет это
    ///    пользователю до отправки формы).
    ///
        ...
        match op {
            "from_refill" => {
                ...
                tokio::task::spawn_blocking(move || -> Result<Option<i64>, AppError> {
                    ...
                    cart_repo.latest_to_refill_source_place(&conn)
                })
                .await
                .map_err(|e| AppError::Internal {
                    source_chain: format!("spawn_blocking: {e}"),
