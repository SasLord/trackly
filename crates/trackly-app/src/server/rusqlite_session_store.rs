...
impl std::fmt::Debug for RusqliteSessionStore {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("RusqliteSessionStore")
...
impl RusqliteSessionStore {
    ...
    pub async fn background_cleanup(&self) -> Result<(), AppError> {
        self.writer
            .execute(|conn| {
                conn.execute("DELETE FROM sessions WHERE expiry_date < unixepoch()", [])
        ...
        match data_opt {
            ...
            Some(bytes) => {
                match rmp_serde::from_slice::<Record>(&bytes) {
                    ...
                    Err(e) => {
                        tracing::warn!("session decode failed, dropping session row: {e}");
